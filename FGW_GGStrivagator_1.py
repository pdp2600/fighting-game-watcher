...
GG Strive data Aggregator script, start as WIP, then create proper module for it
...
def get_char_portrait_col_names(df_columns_ls):
    #Retrieves all the P1 & P2 character portrait match column names
    char_portrait_col_names_ls = []
    for col_name in df_columns_ls:
        if "P_Portrait" in col_name:
            char_portrait_col_names_ls.append(col_name)
            
    return char_portrait_col_names_ls
...
def reduce_match_df_to_agg_df(vid_match_data_df):
    agg_col_names_ls = ['Time_in_Secs', 'Frame_Number_secs']
    char_portraits_ls = get_char_portrait_col_names(list(vid_match_data_df))
    ...
    agg_col_names_ls.extend(char_portraits_ls)
    agg_col_names_ls.extend(other_cols_ls)
    return vid_match_data_df[agg_col_names_ls]

def _find_duel_start_blocks(duel_df, duel_block_threshold):
    duel_index_blocks = []
    prev_index = -1
    start_index = 0
    ...
    for index, row in duel_df.iterrows():
        if (prev_index == -1) and (index == last_index):
            ...
            if index == last_index:
                #Added April 3rd for the ROund 3 unknown round
...
def _evaluate_round_data(round_bool_data_df):
    ...
    #added Mar 29th, making non-round 1 count for double to avoid false positives
...
def _round_number_detect(duel_df, min_delta):
    ...
    for index, row in duel_number_cols_df.iterrows():
        ...
        if row.Starter_Number_2 > 0:
            if (#(row.Starter_Number_2 - row.Starter_Number_1) > min_delta and 
                (row.Starter_Number_2 - row.Starter_Number_3) > min_delta and 
        ...
        if row.Starter_Number_3 > 0:
            if (#(row.Starter_Number_3 - row.Starter_Number_1) > min_delta and 
                (row.Starter_Number_3 - row.Starter_Number_2) > min_delta and 
        ...
        if row.Starter_Number_Final > 0:
            if (#(row.Starter_Number_Final - row.Starter_Number_1) > min_delta and 
                (row.Starter_Number_Final - row.Starter_Number_2) > min_delta and 
...
def _get_paired_lets_rock_block(lets_rock_df, duel_end, duel_to_lets_rock_buffer):
    start = duel_end + 1
...
#it's available
def _round_start_processing(data_for_agg_df, agg_config_dict):
    ...
    duel_index_blocks_ls = _find_duel_start_blocks(duel_detected_df, 
                                                   agg_config_dict[
                                                       'duel_block_index_threshold'])
    for duel_block in duel_index_blocks_ls:
        start = duel_block['start_index']
        end = duel_block['end_index']
        duel_block_df = duel_detected_df.copy()
        duel_block_df = duel_block_df.loc[start: end, ]
        includes_lets_rock = False
        round_num = _round_number_detect(duel_block_df, 
                                         agg_config_dict['duel_number_min_delta'])
...
def _find_orphan_lets_rocks(data_for_agg_df, round_start_df):
    ...
    for index, row in lets_rock_detected_df.iterrows():        
        isOrphaned = True
        #print("Lets Rock Index: {}".format(index))
...
def _lets_rock_dict_to_df(lets_rock_block_ls):
    round_start_df = pd.DataFrame({})
    for block in lets_rock_block_ls:
...
def load_match_csv_into_dataframe(csv_path):
...
def _create_valid_round_dict(cur_row_df, next_row_df):
    cur_row_df = cur_row_df.reset_index()
    next_row_df = next_row_df.reset_index()
    round_dict = {'start_secs': cur_row_df.loc[0,'start_secs'], 
                  'start_index': cur_row_df.loc[0,'start_index'], 
                  'end_secs': next_row_df.loc[0,'end_secs'], 
                  'end_index': next_row_df.loc[0,'end_index'], 
                  'round': cur_row_df.loc[0,'round'], 
                  'winner': 'Unknown', 'winner_via_health': 'Unknown', 
                  'winner_via_tmplate_match': 'Unknown', 
                  'winner_confidence': None, 'character_1P': 'Unknown', 
                  'character_2P': 'Unknown', 
                  'perfect': next_row_df.loc[0,'perfect'], 
                  'double_ko': next_row_df.loc[0,'double_ko'], 
                  'time_out': next_row_df.loc[0,'time_out'], 
                  'draw': next_row_df.loc[0,'draw'], 
...
def _resolve_missing_pair(cur_row_df, next_row_df, prev_row_df, agg_config_dict):
    cur_row_df = cur_row_df.reset_index()
    next_row_df = next_row_df.reset_index()
    prev_row_df = prev_row_df.reset_index()
    next_row_exists = len(list(next_row_df.index)) > 0
    prev_row_exists = len(list(prev_row_df.index)) > 0

    if cur_row_df.loc[0,'block_type'] == 'Starter':
        if next_row_exists == False:
            invalid_note = "Invalid: Round Starter at End of Video"
            return _create_invalid_dict(cur_row_df, invalid_note)
        elif ((cur_row_df.loc[0,'round'] == 'Round_1') and 
        (next_row_df.loc[0,'round'] == 'Round_1')):
            inconl_note = "Missing Ender: Button Check"
            return _create_inconclusive_dict(cur_row_df, next_row_df, 
                                             agg_config_dict, inconl_note)
        elif (((cur_row_df.loc[0,'round'] == 'Round_1') and 
        (next_row_df.loc[0,'round'] == 'Round_2')) or 
        ((cur_row_df.loc[0,'round'] == 'Round_2') and 
        (next_row_df.loc[0,'round'] == 'Round_3')) or 
        ((cur_row_df.loc[0,'round'] == 'Round_3') and 
        (next_row_df.loc[0,'round'] == 'Round_Final'))):
            inconl_note = "Missing Ender: Same Game"
            return _create_inconclusive_dict(cur_row_df, next_row_df, 
                                             agg_config_dict, inconl_note)
        elif (((cur_row_df.loc[0,'round'] == 'Round_2') and 
        (next_row_df.loc[0,'round'] == 'Round_1')) or 
        ((cur_row_df.loc[0,'round'] == 'Round_3') and 
        (next_row_df.loc[0,'round'] == 'Round_1')) or 
        ((cur_row_df.loc[0,'round'] == 'Round_Final') and 
        (next_row_df.loc[0,'round'] == 'Round_1'))):
            inconl_note = "Missing Ender: Different Game"
            return _create_inconclusive_dict(cur_row_df, next_row_df, 
                                             agg_config_dict, inconl_note)
        else:
            ...
            return _create_inconclusive_dict(cur_row_df, next_row_df, 
                                             agg_config_dict, inconl_note)        
    elif cur_row_df.loc[0,'block_type'] == 'Ender':
        if prev_row_exists == False:
            ...
            return _create_inconclusive_dict(cur_row_df, prev_row_df, 
                                             agg_config_dict, inconl_note)
...
def _consolidate_round_data(rounds_df, agg_config_dict):
    ...
    while i < len(index_vector_ls):
        ...
        if index != last_index:
            ...
            if (rounds_df.loc[index,'block_type'] == 'Starter'
                ) and (rounds_df.loc[next_index,'block_type'] == 'Ender'):
...
def _add_anomally_round_df(anomally_round_df, anomally_collection_df, anomally_num):
    ...
    for index in index_ls:
        anomally_round_df.loc[index, 'Anomally_id'] = "Anomally_{}".format(anomally_num)
...
data_capture_fps = 4
default_fps = 4
fps_scaling = data_capture_fps / default_fps

##Probably also need to write a function or modify one to calculate the resoloution
##of the video and scale the template images
agg_config_vals = { 'duel_block_index_threshold': int(16 * fps_scaling), 
                    'duel_to_lets_rock_frame_buffer': int(4 * fps_scaling) + 1, 
                    'ender_block_index_threshold': int(16 * fps_scaling), 
                    'times_up_to_draw_frame_buffer': int(24 * fps_scaling), 
                    'health_after_ender_frame_buffer': int(4 * fps_scaling), 
                    'missing_ender_health_buffer': int(24 * fps_scaling), 
                    #'after_ender_health_detect_buffer': int(4 * fps_scaling), 
                    #'before_ender_health_detect_buffer': int(8 * fps_scaling) + 1, 
                    'player_win_after_ender_buffer': int(56 * fps_scaling),
                    'missing_ender_player_win_buffer': int(18 * fps_scaling), 
                    'missing_starter_index_value': int(8 * fps_scaling), 
                    'missing_starter_secs_value': 2, 
                    'missing_ender_index_value': int(8 * fps_scaling), 
                    'missing_ender_secs_value': 2, 
                    'duel_number_min_delta': 0.04, 
                    'player_win_enders_min_delta': 0.10, 
                    'min_character_delta': 0.05, 
                    'player_health_min_delta': int(200 * fps_scaling), 
                    'missing_ender_player_health_min_delta': 50,
                    'no_health_after_ender_buffer': int(20 * fps_scaling),
                    'no_health_after_ender_delta': 30
                   }
"""

"""#For testing aggregation based on CSVs created with the cv extraction fucntions
#ggst_vid_data_df = load_match_csv_into_dataframe('HC_Detect_Test_4FPS_Vid_Happy_Chaos-Mirror_Jan_21st_2022.csv')
#test_vid_data_df = load_match_csv_into_dataframe('Baiken_mirror_4fps_Perfect-2022-02-03.csv')
#test_vid_data_df_2 = load_match_csv_into_dataframe('HC_Faust_double_Timeout_draw_FInal_round-2022-01-27.csv')
#ggst_vid_data_df = load_match_csv_into_dataframe('HC_Faust_double_Timeout_draw_Final_round-2022-01-27.csv')
#vid_filename = 'HC_Detect_Test_4FPS_Vid_Happy_Chaos-Mirror_Jan_21st_2022.csv'
#vid_filename = 'Baiken_mirror_4fps_Perfect-2022-02-03.csv'
vid_filename = 'HC_Faust_double_Timeout_draw_FInal_round-2022-01-27.csv'

ggst_vid_data_df = load_match_csv_into_dataframe(vid_filename)
"""
##############################################################################


"""
#Exclude this function in main, wrote it to test timer template matching
def timer_template_test_video(video_path, fps_val):
    vid_filename = video_path.split('\\')[1]
    #Loading template images & storing them in memory
    conf.templ_img_dicts_ls = ex.load_tmp_imgs(conf.templ_img_dicts_ls)

    start_time = datetime.now()
    
    #Brute force template matching function
    #ggst_vid_data_df = ex.get_all_tmpl_scores_vid(video_path, conf.templ_img_dicts_ls, 
    #                                                  conf.templ_img_min_val_mappings, 
    #                                              frames_per_sec = fps_val)    
    #Test timer outline template matching
    vid_data_df = ex.get_all_tmpl_scores_vid(video_path, 
                                             conf.timer_templ_img_dicts_ls, 
                                             conf.templ_img_min_val_mappings, 
                                             frames_per_sec = fps_val)
    ...
    timestamp = "{}-{}-{}_{}-{}-{}".format(dt.year, dt.month, dt.day, dt.hour, 
                                            dt.minute, dt.second)
    new_dir = "{}_{}".format(timestamp, output_name)
    #cur_dir = os.curdir
    cur_dir = os.getcwd()
    output_dir = "{}\\output\\{}".format(cur_dir, new_dir)
    os.makedirs(output_dir)
...
#create_json_file(games_agg_df, games_by_rounds_df, wd_path)

##CSVs & JSON done, need to decide about YT markup, 'html', etc, html version
##maybe should display data like round outcomes (perfect, time out, etc) with
##badge/small versions of those template images

#####
#Next steps:
#-Turn the two scripts into modules & create a driver script using them
##-Put the aggregation config into the confige file & write functions where
## 1) runs all the match/aggregation functions & 2) a function to recalculate config
## values based on the FPS (this meta function likely needs to reside in the driver script)
##-maybe an entire function to run the match/extraction & aggregation fucntions
#-Create a longer video of multiple matches of mine first
#-implement Youtube DL module
#-Test Round consolidate error cases with YT videos
#-RUn a bunch of tests on longer videos
#-create at least a couple of outputs (syntax of YT markup for bookmarks by seconds,
# maybe a nice-ish HTML output defaulting to hiding spoilers)

#DONE-Modify the Duel DF to make sure "Times Up" & other enders are 0
#DONE-Do it when collecting the Duel rows in a DF (filtered duel > 0.59)
#DONE-Function to see if there were any missing round starts based on "Let's Rocks"
#DONE-Functions to find ender blocks & data, and then merge with the round starter data 
# for data denoting round start & end
#DONE-Functions to get character data for each round
#DONE-Functions to determine the outcome/winner of each round & the confidence
##DONE-In the case of a missing ender, get frames 1 before next starter & based on the
## missing ender health detect buffer
#DONE-Functions to assemble the rounds as games with outcomes if possible to be known
#DONE-Run tests with the output csvs I already have as a double check
#DONE-Make the move to comment out any match fields which are not used in the aggregation


"""
with open('Games_with_round_data__2022-3-9_20-19-47.json') as json_file:
    data = json.load(json_file)
    print(data)
"""

"""
games_json = {'original_vid_filename': orignal_vid_file,
              'youtube_video_link': youtube_link,
              'games': [{'id': game_id, 'start_secs': start_secs, 'end_secs': end_secs,
                        'start_index': start_index, 'end_index': end_index, 
                        'total_rounds': total_rounds, 'character_1P': character_1P,
                        'character_2P': character_2P, 'winner': winner, 
                        'player_1_rounds_won': player_1_rounds_won, 
                        'player_2_rounds_won': player_2_rounds_won, 
                        'inconclusive_data': inconclusive_data, 
                        'inconclusive_note': inconclusive_note,
                        'rounds': [{'round': _round, 'start_secs': start_secs, 
                                    'end_secs': end_secs, 'start_index': start_index, 
                                    'end_index': end_index, 
                                    'character_1P': character_1P, 
                                    'character_2P': character_2P, 
                                    'winner': winner, 
                                    'winner_confidence': winner_confidence, 
                                    'winner_via_health': winner_via_health, 
                                    'winner_via_tmplate_match': winner_via_tmplate_match, 
                                    'perfect': perfect, 'double_ko': double_ko,
                                    'time_out': time_out, 'draw': draw, 
                                    'inconclusive_data': inconclusive_data, 
                                    'inconclusive_note': inconclusive_note}]
                        }
                       ]
...
#For figuring out the last row index in a DF: df.index.stop -1 
#To check if a df is empty: len(df.index)


"""

test_key = test_key.replace("_1P_Portrait", "") if (
    '1P' in test_key) else test_key.replace("_2P_Portrait", "")
##Example code for itterating and accessing df values
for index, row in consolidated_rounds_df.iterrows():
    print("Start: {}, End: {}".format(row.start_index, row.end_index))

test_df.append(pd.DataFrame(round_dict, index = [start]), sort=False)

##Example code for finding the max val in a dict
test_dict = {'Anji': 3, 'Sol': 3, 'Gio': 0}
max(test_dict, key=test_dict.get)
test_dict['Baiken'] = 10


round_start_test_1 = round_start_blocks_df.copy()
round_start_test_1.loc[159, 'end_index'] = 161
orphaned_lets_rocks_df = _find_orphan_lets_rocks(ggst_data_for_agg_df, 
                                                 round_start_test_1)

if len(orphaned_lets_rocks_df) > 0:
    lets_rock_rounds = _find_lets_rock_start_blocks(orphaned_lets_rocks_df)
    round_start_test_1 = round_start_test_1.append(lets_rock_rounds)
    round_start_test_1 = round_start_test_1.sort_index(axis = 0)
    
"""
"""
def _create_round_start_dict(start_index, end_index, start_secs, end_secs, round_num):
    return {'start_index': start_index, 'end_index': end_index, 
            'start_secs': start_secs, 'end_secs': end_secs, 'round': round_num
            }

def _consolidate_lets_rock_blocks(lets_rock_rows_df):
    lets_rocks_blocks_df = pd.DataFrame({})
    start_index = 0
    prev_index = -1
    
    if len(lets_rock_rows_df) == 1:
        start_index = list(lets_rock_rows_df.index)[0]
        round_dict = _create_round_start_dict(start_index, start_index, 
                                              lets_rock_rows_df.loc[start_index, 'Time_in_Secs'], 
                                              lets_rock_rows_df.loc[start_index, 'Time_in_Secs'], 
                                              "Unknown")      
        lets_rocks_blocks_df = pd.DataFrame(round_dict, index = [start_index])
        
        return lets_rocks_blocks_df
    
    else:    
        for index, row in lets_rock_rows_df.iterrows():
            if prev_index == -1:
                start_index = index
                prev_index = index
            else:
                is_Last_row = len(lets_rock_rows_df.loc[index: ,]) == 1
                index_delta = index - prev_index
            
                if (index_delta == 1) and is_Last_row:
                    start_secs = lets_rock_rows_df.loc[start_index, 'Time_in_Secs']
                    end_secs = lets_rock_rows_df.loc[index, 'Time_in_Secs']
                    round_dict = _create_round_start_dict(start_index, index, 
                                                          start_secs, end_secs, 
                                                          "Unknown")
                    lets_rocks_blocks_df = lets_rocks_blocks_df.append(pd.DataFrame(round_dict, 
                                                                                    index = [start_index]), 
                                                                       sort=False)
                
                elif (index_delta == 1) and not is_Last_row:
                    prev_index = index

                else:
                    start_secs = lets_rock_rows_df.loc[start_index, 'Time_in_Secs']
                    end_secs = lets_rock_rows_df.loc[prev_index, 'Time_in_Secs']
                    round_dict = _create_round_start_dict(start_index, prev_index, 
                                                          start_secs, end_secs, 
                                                          "Unknown")                    
                    lets_rocks_blocks_df = lets_rocks_blocks_df.append(pd.DataFrame(round_dict, 
                                                                                    index = [start_index]), 
