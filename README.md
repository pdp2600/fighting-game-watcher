...
=======
		<li>With an Anaconda prompt window open, while being in the directory you'd like the _fighting-game-watcher_ folder to be created in, and the project's environment activated, setup the environment's local git with your Github username and email associated with the acocunt: <pre>git config --global user.email "my_email523@emaildomain.ca"</pre><pre>git config --global user.name "GitHubUserName"</pre> You'll also be prompted for the password the first time in a session a command is used which requires authentication.</li>
>>>>>>> 975aec0d456bc71b3e977531c6b8108ed468af81
