...
#Finds the videos total duration in seconds based on raw data extracted
...
def _create_game_dict_for_json(game_input_df, game_rounds_df)->dict:
    ...
    return game
def _create_anomaly_bookmark_dict(anomallies_df, df_index)->dict:
    return {'id': str(anomallies_df.loc[df_index, 'anomaly_id']), 
            'time': int(anomallies_df.loc[df_index, 'start_secs']), 
            'round': str(anomallies_df.loc[df_index, 'round']), 
            'character_1P': str(anomallies_df.loc[df_index, 'character_1P']), 
            'character_2P': str(anomallies_df.loc[df_index, 'character_2P'])
            }

def _create_game_round_bookmark_dict(rounds_or_games_df, df_index)->dict:
    ...
    if len(anomallies_df) > 0:
        ...
        if len(anomallies_df) > 0:
            for index, row in anomallies_df.iterrows():
                round_bookmark:dict = _create_anomaly_bookmark_dict(anomallies_df, 
                                                                    index)
...
def _create_youtube_chapters(chapter_bookmarks_df, prepend_id:bool=False, 
    ...
    chapters:str = "" 
    #If 1st game/round is 10secs or sooner, then it needs to be 0:00 by YT chapter rules
